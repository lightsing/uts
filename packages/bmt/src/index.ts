...
export class UnhashedFlatMerkleTree {
    constructor(
     ...
     */
    public finalize(factory: HasherFactory): UnorderedMerkleTree {
        ...
        return new UnorderedMerkleTree(nodes, len);
    }
}
...
export class UnorderedMerkleTree {
     ...
     */
    public static new(data: Uint8Array[], factory: HasherFactory): UnorderedMerkleTree {
     ...
     */
    public static newUnhashed(prehashedLeaves: Uint8Array[], factory: HasherFactory): UnhashedFlatMerkleTree {
        ...
        nodes[0] = new Uint8Array(factory.outputSize);

        // Prepare leaves block
        const leavesBlock = new Array<Uint8Array>(len);
        for (let i = 0; i < len; i++) {
            if (i < rawLen) {
                if (prehashedLeaves[i].length !== factory.outputSize) {
                    throw new Error(`Invalid leaf at index ${i}: expected length ${factory.outputSize}, got ${prehashedLeaves[i].length}`);
                }
                leavesBlock[i] = prehashedLeaves[i];
            } else {
                // Pad with default (zeroed) hash
                leavesBlock[i] = new Uint8Array(factory.outputSize);
            }
        }
    ...
    public asRawBytes(): Uint8Array {
        const hashLength = this.nodes[1].length;
        const totalSize = this.nodes.length * hashLength;
        const bytes = new Uint8Array(totalSize);

        for (let i = 0; i < this.nodes.length; i++) {
            bytes.set(this.nodes[i], i * hashLength);
        }
     ...
     */
    public static fromRawBytes(bytes: Uint8Array, hashLength: number): UnorderedMerkleTree {
        if (bytes.length % hashLength !== 0) {
            throw new Error("Bytes length must be a multiple of hashLength");
        }
        const totalNodes = bytes.length / hashLength;
        if (totalNodes % 2 !== 0) {
        ...
        for (let i = 0; i < totalNodes; i++) {
            nodes[i] = bytes.slice(i * hashLength, (i + 1) * hashLength);
        }
 ...
 */
export class SiblingIter implements IterableIterator<{ position: NodePosition; sibling: Uint8Array }> {
    ...
    public next(): IteratorResult<{ position: NodePosition; sibling: Uint8Array }> {
        if (this.current <= 1) {
    ...
    public [Symbol.iterator](): IterableIterator<{ position: NodePosition; sibling: Uint8Array }> {
        return this;
    }
