...
<template>
  <div
    class="space-y-0.5"
    :style="{ paddingLeft: currentDepth > 0 ? '16px' : '0' }"
  >
    <div v-for="(step, i) in steps" :key="i" class="group">
      ...
      <div v-if="step.op === 'ATTESTATION'">
        ...
        >
          <component
            :is="expandedAttestations.has(i) ? ChevronDown : ChevronRight"
        ...
        <Transition name="fade">
          <div
            v-if="expandedAttestations.has(i)"
            class="ml-5 mt-1 rounded-lg border border-glass-border/50 bg-surface/30 px-3 py-2"
          >
